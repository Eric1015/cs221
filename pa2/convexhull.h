...
// returns whether the third point is on the left side of the function given the
// function, the point, and the direction it points to
// helper function for ccw
// p is the Point p3 from ccw
// p1 and p2 are also Points p1 and p2 from ccw
// p1 and p2 here are used to determine the direction of the line
// in order to determine the direction of the function
bool isOnLeft(double slope, double intercept, Point p, Point p1, Point p2);

// returns whether the third point is above the function given the 
// function and the point
// helper function for isOnLeft
bool isAbove(double slope, double intercept, Point p);

// returns whether the third point is below the function given the 
// function and the point
// helper function for isOnLeft
bool isBelow(double slope, double intercept, Point p);

// swap the first element of v with the element of smallest y-coordinate
// helper function for sortByAngle
void swapMinY(vector<Point>& v);

// returns the radian made by line p1p2 and x-axis
// helper function for sortByAngle
double radian_X_axis(Point p1, Point p2);

// returns true if provided three points are on the one straigt line
// helper function for getConvexHull
bool isOnStraightLine(Point p1, Point p2, Point p3);

// add point p to vector v if vector v still does not have p in it
// helper function for getConvexIntersection and addPointsInside
